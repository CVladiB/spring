...
public class AuthorDaoJdbc implements AuthorDao{
    ...
    public void create(@NonNull Author author) {
        String sql = "insert into author (author_name, author_surname)" +
                "values ( :name,:surname)";
    ...
    public Author read (int id) {
        String sql = "select author_id, author_surname, author_name from author where author_id = :id";
        return jdbc.queryForObject(sql,Map.of("id", id), new AuthorMapper());
    }

    @Override
    public List<Author> read () {
        String sql = "select author_id, author_surname, author_name from author";
        return jdbc.getJdbcOperations().query(sql, new AuthorMapper());
    }

    @Override
    public void update(@NonNull Author author){
        String sql = "update author set author_surname = :surname, author_name = :name where author_id = :id";
